...
=============================================================
Purpose: Test Qwen VL models on unstructured medical documents (PDF, TXT)
Models Supported:
  - Qwen3-VL-4B-Thinking (reasoning, slower)
  - Qwen3-VL-4B-Instruct (balanced, 37s/page)
  - Qwen2.5-VL-3B-Instruct (fastest, target <20s/page)

GPU: NVIDIA RTX 3090 24GB (test environment)

IMPORTANT COMPATIBILITY NOTES:
- Qwen2.5-VL-3B has different architecture than Qwen3-VL-4B
- INT8 quantization NOT compatible with Qwen2.5 (use FP16 instead)
- Qwen2.5 uses Qwen2VLProcessor/Qwen2VLForConditionalGeneration classes
- Model weights show UNEXPECTED/MISSING keys (this is NORMAL for cross-version)
...
# Transformers for Qwen VL models (both 2.5 and 3.0 versions)
try:
    from transformers import (
        Qwen3VLProcessor, Qwen3VLForConditionalGeneration,  # For 3.0 models
        Qwen2VLProcessor, Qwen2VLForConditionalGeneration,  # For 2.5 models
        BitsAndBytesConfig
    )
...
# Model Selection: Choose Qwen VL variant
# - "thinking": Qwen3-VL-4B-Thinking (slower, reasoning-capable, ~70s/page)
# - "instruct": Qwen3-VL-4B-Instruct (balanced, 37s/page) ← PROVEN PRODUCTION
# - "2b-instruct": Qwen3-VL-2B-Instruct (faster, ~20-25s/page target) ← TESTING NOW
# - "2.5-3b-instruct": Qwen2.5-VL-3B-Instruct (incompatible - architecture issues)
MODEL_VARIANT = "2b-instruct"  # Options: "thinking", "instruct", "2b-instruct", "2.5-3b-instruct"
...
class ProcessingResult:
    ...
    ocr_engine: Optional[str] = None           # Dynamic model name
    ...
    def to_postgres_json(self) -> Dict:
        ...
        return {
            "document": {
                ...
                "ocr_engine": self.ocr_engine or "Unknown"  # ✅ FIXED: Now dynamic
            },
...
def extract_medical_entities_regex(text: str) -> List[Dict[str, Any]]:
    ...
    # ENHANCED FOR 2B: Handles flexible spacing: "15.8 g/dL ( 13.0 - 18.0 )"
    lab_test_pattern = re.compile(
        ...
        r'([a-zA-Z/%^0-9\-\.]+)\s*'  # Unit + optional spaces (FIX: allow space before paren)
        r'\(\s*([^\)\n]+?)\s*\)',  # Reference range with optional internal spaces
    ...
    # STRICT: Avoid matching standalone "Negative" table cells
    qualitative_pattern = re.compile(
        r'^([A-Z][A-Za-z ,\-/()#]+?(?:  +[\u4e00-\u9fff]+)?)'  # Test name + Chinese
        r'\s{2,}'  # Multiple spaces (2+)
    ...
    for match in qualitative_pattern.finditer(text):
        ...
        # STRICT VALIDATION to filter noise:
        # 1. Test name must be >8 chars (avoid "Negative: Negative")
        # 2. Test name cannot equal result (skip "Negative: Negative")
        # 3. No newlines, not ALL CAPS, not generic starters
    ...
    # ENHANCED FOR 2B: Flexible spacing around parentheses
    ...
    # Pattern: "血红蛋白 15.8 g/dL ( 13.0 - 18.0 )" - 2B model outputs these
    chinese_only_pattern = re.compile(
        r'^([\u4e00-\u9fff]{2,})\s+'  # Chinese test name (2+ chars)
        r'([*])?\s*'  # Optional asterisk
        r'([<>≤≥]?\s*\d+\.?\d*)\s+'  # Value
        r'([a-zA-Z/%^0-9\-\.]+)\s*'  # Unit + optional spaces
        r'\(\s*([^\)\n]+?)\s*\)',  # Reference range
        re.MULTILINE | re.UNICODE
    )
...
class Qwen3VLEngine:
    ...
    def __init__(self, model_variant: str = "instruct", optimization_tier: str = "tier1", use_model_ner: bool = False):
        ...
        if model_variant == "thinking":
            ...
            self.is_qwen25 = False
        elif model_variant == "2b-instruct":
            model_name = "Qwen/Qwen3-VL-2B-Instruct"
            model_display = "Qwen3-VL-2B-Instruct (FASTER - 2B params)"
            self.is_qwen25 = False
        elif model_variant == "2.5-3b-instruct":
            model_name = "Qwen/Qwen2.5-VL-3B-Instruct"
            model_display = "Qwen2.5-VL-3B-Instruct (INCOMPATIBLE - 3B params)"
            self.is_qwen25 = True
        else:
            raise ValueError(f"Invalid model_variant: {model_variant}. Use 'thinking', 'instruct', '2b-instruct', or '2.5-3b-instruct'")
        
        self.model_variant = model_variant
        self.optimization_tier = optimization_tier
        self.model_name = model_name
        self.model_display_name = model_display  # ✅ Store for later reference
        ...
        try:
            # Load processor (different for Qwen2.5 vs Qwen3)
            ...
            if optimization_tier in ["tier1", "tier2"]:
                # INT8 quantization - ONLY for Qwen3 models (incompatible with Qwen2.5)
                if not self.is_qwen25:
                    ...
                    # Qwen2.5 models: Use FP16 (INT8 incompatible)
                    print("      ⚠ Qwen2.5 models don't support INT8 quantization")
                    print("      Using FP16 instead (still fast due to smaller model)")
                    model_kwargs["torch_dtype"] = torch.float16 if self.device == "cuda" else torch.float32
                
                # Try Flash Attention 2 for 1.5-2x speedup (works for both)
                try:
            ...
            # Load model (different class for Qwen2.5 vs Qwen3)
            if self.is_qwen25:
                # Qwen2.5: Allow mismatched sizes (merger MLP differences)
                self.model = Qwen2VLForConditionalGeneration.from_pretrained(
                    model_name,
                    ignore_mismatched_sizes=True,  # Allow merger MLP size differences
                    **model_kwargs  # Already includes trust_remote_code
                )
            else:
...
- Task acknowledgment ("Got it", "Sure", "Okay")
- Process narration ("Now I'll...", "First, let me...")
- Self-referential language ("I see", "I notice", "Let me")
- Observational commentary ("Looking at the image...")
- Meta-process comments ("As instructed...", "Per your requirements...")
- Interpretive commentary ("This value is abnormal")

════════════════════════════════════════════════════════════════════════════════
 OUTPUT FORMAT
════════════════════════════════════════════════════════════════════════════════

Extract text line by line as it appears in the image. NO template markers.
Separate pages with: --- PAGE BREAK ---

DO NOT output placeholders like "[Facility]", "[SECTION HEADER - e.g., XX]", or "[value or empty]".
If text is redacted/blacked out, write only: [REDACTED]

Output ONLY what you see in the image. Nothing more
...
════════════════════════════════════════════════════════════════════════════════"""
            ...
            # Process inputs differently for Qwen2.5 vs Qwen3
            if self.is_qwen25:
                # Qwen2.5-VL: Simpler API - text as string, image as PIL object
                ...
                # Pass as single items (not lists) - processor adds batch dimension
                inputs = self.processor(
                    text=text,  # String, not list
                    images=image_obj,  # Single PIL Image, not list
                    return_tensors="pt"
                )
            else:
                # Qwen3-VL: Apply chat template first, then use process_vision_info
                text = self.processor.apply_chat_template(
                    messages, tokenize=False, add_generation_prompt=True
                )
            ...
            # Move to device (handle all tensor types properly)
            inputs_on_device = {}
            for k, v in inputs.items():
                if isinstance(v, torch.Tensor):
                    inputs_on_device[k] = v.to(self.device)
                elif isinstance(v, list) and len(v) > 0 and isinstance(v[0], torch.Tensor):
                    # Handle list of tensors (e.g., image_grid_thw for Qwen2.5)
                    inputs_on_device[k] = [t.to(self.device) for t in v]
                else:
    ...
    def process_txt(self, txt_path: str) -> ProcessingResult:
        ...
        try:
            ...
            result = ProcessingResult(
                ...
                ocr_engine=self.vision_engine.model_display_name,  # ✅ Dynamic
            ...
            return ProcessingResult(
                ...
                file_hash=file_hash,
                ocr_engine=self.vision_engine.model_display_name if hasattr(self, 'vision_engine') else "Unknown",
                processing_time=processing_time,
                vram_used_mb=gpu['allocated_mb'],
                error=str(e)
            )
