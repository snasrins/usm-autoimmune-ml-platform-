#!/usr/bin/env python3
"""
Standalone Unstructured Data Pipeline - GPU Test
================================================
Purpose: Test Qwen3-VL-4B-Thinking on unstructured medical documents (PDF, TXT)
...
USE_MODEL_BASED_NER = True  # Set to True for comprehensive entity extraction
...
class Qwen3VLEngine:
    ...
    def __init__(self, model_name: str = "Qwen/Qwen3-VL-4B-Thinking", use_model_ner: bool = False):
        print(f"Loading {model_name}...")
        print("   This may take 2-3 minutes on first run (downloading model)...")
        
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model_name = model_name
        self.use_model_ner = use_model_ner  # Enable scalable model-based entity extraction
        
        if use_model_ner:
            print("   \u2699\ufe0f Model-based NER enabled (slower but handles any medical terms)")
        ...
        try:
            ...
            print(f"✅ Qwen3-VL-4B-Thinking loaded on {self.device}")
            
            # Check VRAM usage after loading
            if self.device == "cuda":
                vram_mb = torch.cuda.memory_allocated(0) / (1024**2)
                print(f"   Model VRAM: {vram_mb:.2f} MB ({vram_mb/1024:.2f} GB)")
            
        except Exception as e:
            print(f"❌ Failed to load Qwen3-VL-4B-Thinking: {e}")
            raise
...
════════════════════════════════════════════════════════════════════════════════"""
            ...
            # Generate with strict parameters to minimize thinking
            output_ids = self.model.generate(
                **inputs,
                max_new_tokens=2048,  # Increased for full document
...
Output the JSON array immediately. Nothing else."""
            ...
            try:
                ...
                if json_start >= 0 and json_end > json_start:
                    ...
                    print(f"   ⚠️ No JSON array found in NER output")
            ...
            print(f"   ⚠️ Model-based NER failed: {e}")
            import traceback
            traceback.print_exc()
            return []
...
class DocumentProcessor:
    ...
    def process_pdf(self, pdf_path: str) -> ProcessingResult:
        ...
        try:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    if len(text.strip()) > 50:
                        ...
                        print(f"   ⚠️ Page {page_num}: Poor text quality, needs OCR")
            ...
            if failed_pages:
                print(f"\n   Step 2: Running Qwen3-VL-4B-Thinking OCR on {len(failed_pages)} pages...")
                ...
                for page_num in failed_pages:
                    ...
                    if page_idx < len(images):
                        ...
                        entities = vision_result.get("medical_entities", [])
                        conf = vision_result.get("confidence", 0.85)
...
class UnstructuredPipeline:
    ...
    def __init__(self, output_dir: str = "./pipeline_output"):
        ...
        print("\n" + "="*80)
        print(" USM AUTOIMMUNE - UNSTRUCTURED DATA PIPELINE")
        print("="*80)
...
def main():
    ...
    for fp in file_paths:
        ...
        if not path.exists():
            ...
            print(f"⚠️ Unsupported file type: {fp} (only .pdf and .txt supported)")
        else:
